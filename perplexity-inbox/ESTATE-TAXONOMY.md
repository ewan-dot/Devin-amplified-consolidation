...
| :--- | :--- | :--- | :--- | :--- |
| `amplified_brain` | **ACTIVE** | **ACTIVE** | Canonical, single source of truth for the Sovereign Business Brain. Shares the PostgreSQL engine on the same `cove-postgres` container. | • Graph `business_brain` (89,385 nodes): Entity relations, client models, extracted rules.<br>• Graph `compound_design` (53,959 nodes): Architecture codebases, structural nodes.<br>• Tables `research_findings`, `design_patterns`, `pipeline_runs`. |
| `cove` (Renamed: `Telemetry and temporal production database (prior name - cove) - 30-06-2026`) | **INACTIVE** (Clean target) | **INACTIVE** | Telemetry and temporal production database. Shares the PostgreSQL engine, pg graph (Apache AGE), and vector capabilities of the `cove-postgres` container but does not use them for production orchestration. | • Tables `agent_runs`, `build_plans` (24 rows), `tasks` (218 rows).<br>• Table `system_prompts` (685 rows): Base prompts for agent fleet.<br>• Table `kaizen_applied_changes` (661 rows): Self-healing improvements. |
