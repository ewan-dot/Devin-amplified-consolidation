...
5.  **Taxonomy Read-Only Guard:** This document (`ESTATE-TAXONOMY.md`) is strictly read-only for executing agents. Agents are forbidden from writing to it directly. All environmental modifications (new paths, container ports, databases, files) must be submitted to Vellum and GitHub. The Python/Rust Vellum plumbing automatically parses these records and compiles the master taxonomy updates.
